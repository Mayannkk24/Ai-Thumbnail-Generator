  ...
  return (
    <div className='relative mx-auto w-full max-w-2xl'>
        <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}>
            {/* IsLoading */}
            {isLoading && (
                <div className='absolute inset-0 flex flex-col items-center justify-centre gap-4 bg-black/25'>
                    <Loader2Icon className='size-8 animate-spin text-zinc-400'/>
                    <div className='text-centre'>
                        <p className='text-sm font-medium text-zinc-200'> AI Is Generating Your Thumbnail...</p>
                        <p className='m-1 text-xs text-zinc-400 '>This may Take 10-20 seconds</p>
                    </div>
                </div>
          ...
          {/* ImagePreview */}
            ...
            {!isLoading && !thumbnail?.image_url &&(
                <div className='absolute inset-0 m-2 flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-white/20 bg-black/25'>
                    <div className='max-sm:hidden flex size-20 items-center justify-center rounded-full bg-white/30'>
                        <ImageIcon className='size-10 text-white opacity-50'/>
                    </div>
                    <div className='px-4 text-centre'>
                        <p className='font text-zinc-200'> Generate your First Thumbnial </p>
                           <p  className='mt-1 text-xs text-zinc-400'>Fill out form and Click Generate</p>
                    </div>
                </div>
