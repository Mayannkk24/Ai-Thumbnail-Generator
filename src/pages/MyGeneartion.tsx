...
const MyGeneartion = () => {
  ...
  return (
    <>
      <SoftBackdrop/>
      <div className='mt-32 min-h-screen px-6 md:px-16 lg:ppx-24 xl:px-32'>
        <div className='mb-8'>
          <h1 className='text-2xl font-bold text-zinc-200'>My Generations</h1>
          <p className='text-sm text-zinc-400 mt-1'>Views and Manage all your AI-Generated Thumbnails,</p>
        </div>
        ...
        {!loading && thumbnails.length === 0 && (
          <div className='text-centre py-24'>
              ...
              <div key={thumb._id} onClick={()=> navigate (`/generate/${thumb._id}`)} className='mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid'>
                ...
                <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                  ...
                  <div onClick={(e)=>e.stopPropagation()} className='absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5'>
                    ...
                    <TrashIcon onClick={()=> handleDelete(thumb._id)} className='size-6 bg-black/50 p-1 rounded hover:bg-pink-500 transition-all'/>

                    <DownloadIcon onClick={()=>handleDowanload(thumb.image_url!)} className='size-6 bg-black/50 p-1 rounded hover:bg-pink-500 transition-all'/>
                     
                     <Link target='_blank' to={`/preview?thumbnail_url=${thumb.image_url}&title=${thumb.title}`}>
                    <ArrowRightIcon className='size-6 bg-black/50 p-1 rounded hover:bg-pink-500 transition-all'/>
                    </Link>
                  </div>
              </div>
             )
